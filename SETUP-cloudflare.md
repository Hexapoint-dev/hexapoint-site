...
**メール送信（Resend）**
`RESEND_API_KEY` · `CONTACT_TO` · `CONTACT_FROM` · `RESEND_WEBHOOK_SECRET`
（配信ステータス追跡に必要。14章参照）
...
GraphQL Analytics API / D1 REST API のフィールド名は今回の実装時点のドキュメントに
基づいていますが、Cloudflare 側で仕様が変わることがあります。もしタブに
「エラー: cf_graphql_failed: ...」や「エラー: cf_d1_database_info_unexpected_shape: ...」
のような表示が出たら、そのエラーメッセージ全文をコピーして開発者に共有してください
（Cloudflare 側が返す正確なフィールド名がエラーの中に含まれているので、修正が簡単です）。
D1のストレージ表示だけがエラーでも、読み取り/書き込み行数の表示は別の仕組みなので
引き続き正常に表示されます。Zoho・Resend・Cloudflare Pages の部分にはこの種のリスクは
ありません。
...
## 14. Resend Webhook — 配信ステータス追跡＋送信数カウンターの正確化

このサイトが送るメール（注文確認・銀行振込通知・返金/異議申し立て/支払い失敗の
アラート・お問い合わせ通知・ステータス変更通知）は、以前は「送信APIが成功を
返した瞬間」に自社カウンターを+1するだけでした。これだと、新しい送信経路を
追加したときにカウンターへの加算を書き忘れると数字がズレる、という弱点があります。

Resend の Webhook を使うと、Resend 側が実際にメールを処理した時点で
`email.sent` イベントが飛んでくるので、**カウンターの加算はこのイベント1本に
一元化**されました（`functions/api/resend-webhook.js`）。さらに `email.delivered`
（到達）・`email.bounced`（不達）・`email.complained`（迷惑メール報告）も同時に
受け取り、D1の `email_log` テーブル（`migrations/0008_email_log.sql`）を更新する
ので、管理画面の注文詳細・お問い合わせ詳細に「このメール、実際に届いた？」が
バッジで表示されるようになります。

### 仕組み（Stripe Webhookと同じ考え方）

Resend は Webhook の署名検証に Svix という仕組みを使っています（Stripe とは
別方式）。`functions/_shared/resend.js` の `verifyResendSignature()` が
`svix-id` / `svix-timestamp` / `svix-signature` ヘッダーを検証し、正規の
Resend からのリクエストであることを確認してから処理します。

### 手順A — Resend Dashboard で Webhook を作成

1. https://resend.com/webhooks にログイン → **Add Webhook**。
2. **Endpoint URL**: `https://www.hexapoint-jp.com/api/resend-webhook`
3. **Events** で以下にチェック：
   - `email.sent`（必須 — カウンター集計に使用）
   - `email.delivered`
   - `email.delivery_delayed`
   - `email.bounced`
   - `email.complained`
4. **Add** で保存 → 作成された Webhook の詳細画面で **Signing Secret**
   （`whsec_...`）をコピー。

### 手順B — Cloudflare に環境変数を設定

Cloudflare Dashboard → Workers & Pages → `hexapoint`（Pagesプロジェクト）→
Settings → Environment variables に追加：

| 変数名 | 値 | 種別 |
|---|---|---|
| `RESEND_WEBHOOK_SECRET` | 手順Aでコピーした `whsec_...` | Secret |

設定後、再デプロイしてください（Deployments → 最新デプロイ → Retry deployment）。

### 手順C — D1マイグレーションの適用

まだ実行していなければ：
```
wrangler d1 execute <DB名> --file=migrations/0008_email_log.sql --remote
```

### 動作確認

1. 実際にテスト送信をトリガーする（例: お問い合わせフォームから1件送信）。
2. Resend Dashboard → 該当 Webhook → **Recent deliveries** に `200 OK` が
   記録されることを確認。
3. `/admin.html` → お問い合わせタブ → 今送ったメッセージを開く →
   「通知メール状況」に「送信済み」バッジが表示される（数秒後に再度開くと
   「到達」に変わっているはず）。
4. System Status タブの Resend カードの数字が、この1通の分だけ増えている
   ことを確認。

⚠️ **`RESEND_WEBHOOK_SECRET` を設定するまでの間**: 送信数カウンターは増えなく
なります（0のまま、または止まったまま）。ゼロに見えたら「正常に動いていない」
サインなので分かりやすいですが、念のため、この章の設定を最後まで終えてから
本番の送信数を確認してください。

---

## 13. Oracle Object Storage — D1バックアップの保存容量表示

D1データベース（`orders` 等の全テーブル）は、Cloudflare とは別の
**Oracle Cloud Always Free** の Object Storage に、毎日自動でバックアップされています
（`functions/` の外にある独立プロジェクト `hexapoint-d1-backup-worker/` — Cloudflare
Worker + Cron Trigger が毎日 D1 の全テーブルを JSON に書き出し、gzip 圧縮して
Oracle にアップロードします。この Worker は Pages プロジェクトとは別にデプロイされて
いるので、このリポジトリの `functions/` フォルダには含まれません）。

このセクションで設定するのは、そのバックアップの**保存状況を管理画面で見るだけ**の
機能です（`functions/_shared/oracle.js` → `functions/api/admin/status.js`）。
バックアップ自体を止めたり作ったりする機能ではありません。

### 仕組み

Oracle の Object Storage には、OCI SDK や署名付きリクエストを使わなくても、
**Pre-Authenticated Request（PAR）** という「アクセス権付きの特別なURL」を発行できます。
バックアップの**アップロード用**には「書き込み専用」PAR を既に使っています
（`hexapoint-d1-backup-worker` 側の `ORACLE_PAR_URL`）。

今回はそれとは**別に**、**読み取り専用＋一覧取得専用**の PAR をもう1つ発行し、
この Pages プロジェクトの環境変数に設定します。2つの PAR を分けているのは、
「バックアップを読み取れる権限」と「バックアップを上書き・削除できる権限」を
混ぜないためです（片方が万一漏れても、もう片方の操作はできません）。

### 手順A — 読み取り専用 PAR を発行する

1. OCI Console → **Storage → Object Storage & Archive Storage → Buckets**。
2. バックアップに使っているバケット（例: `hexapoint-backups`）を開く。
3. 左メニューの **Pre-Authenticated Requests → Create Pre-Authenticated Request**。
4. 設定内容：
   - Name: 例 `d1-backup-status-reader`
   - PAR Target: **Bucket**
   - Access Type: **Permit object reads**
   - **Enable Object Listing** に必ずチェックを入れる
     （これがないと使用容量の一覧が取得できません）
   - Expiration: 任意（1年後など）— 期限が切れると管理画面のこの項目だけが
     「エラー」表示になります（バックアップ自体には影響しません）
5. **Create** → 表示された URL を**その場でコピー**（二度と表示されません）。
   `.../o/` で終わるURLです。

⚠️ アップロード用の PAR（`ORACLE_PAR_URL`）とは絶対に混同しないこと。
今回発行したのは読み取り専用の**別の** PAR です。

### 手順B — Cloudflare Pages に環境変数を設定
...
| 変数名 | 値 | 種別 |
|---|---|---|
| `ORACLE_STORAGE_LIST_PAR_URL` | 手順Aで発行したURL | Secret（推奨） |

設定後、Pages を再デプロイ（環境変数の変更は次回デプロイから反映されます。
すぐ反映させたい場合は「Retry deployment」で最新のデプロイを再実行してください）。

### 動作確認

1. `/admin.html` にログイン → 「システム状況 / Status」タブを開く。
2. 一番下に **Oracle Object Storage** カードが表示され、使用容量（例: `12.3 MB / 10.0 GB`）と、
   バックアップ数・最新バックアップ日時が表示されます。
3. `ORACLE_STORAGE_LIST_PAR_URL` が未設定のうちは「未設定です」とだけ表示され、
   他のカード（Zoho / Resend / Cloudflare）には一切影響しません。

> Oracle Always Free の Object Storage 上限は執筆時点で Standard 10GB です。
> 変更されている可能性があるので、気になれば https://www.oracle.com/cloud/free/
> で確認してください（`functions/_shared/oracle.js` の `ORACLE_FREE_LIMITS` を
> 実際の値に合わせて調整できます）。
