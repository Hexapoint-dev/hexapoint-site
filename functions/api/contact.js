...
import { insertContactMessage, logEmailSend } from "../_shared/db.js";
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    // primary delivery path, this is additive. The returned id links the
    // notification email below (logEmailSend) back to this specific message.
    let contactMessageId = null;
    if (env.DB) {
      try {
        const inserted = await insertContactMessage(env, { name, email, service, message });
    ...
    await logEmailSend(env, {
      ...
      relatedType: contactMessageId ? "contact_message" : undefined,
      relatedId: contactMessageId || undefined,
