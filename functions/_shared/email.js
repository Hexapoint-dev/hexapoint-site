...
//
// Send-quota tracking (trackResendSend, see usage.js) is NOT called here
// anymore -- it's driven entirely by the email.sent event on
// functions/api/resend-webhook.js now, so the counter can't drift due to a
// send path that forgets to instrument itself. What every function below
// does instead is logEmailSend(), which records the Resend email ID so the
// webhook has something to update later, and so the admin panel can show
// delivery status next to the order/message the email was about.
...
export async function sendOwnerAlert(env, { subject, title, titleEn, rows, urgent, emailType, relatedType, relatedId }) {
...
<body style="margin:0;padding:0;background:${PAPER};">
  ...
  try {
    ...
    await logEmailSend(env, {
      resendEmailId: sent && sent.id,
      emailType: emailType || "owner_alert",
      relatedType,
      relatedId,
      recipient: env.CONTACT_TO,
    });
    return { ok: true };
  } catch (err) {
...
<body style="margin:0;padding:0;background:${PAPER};">
  ...
  try {
    ...
    await logEmailSend(env, {
      resendEmailId: sent && sent.id,
      emailType: "bank_order_notification",
      relatedType: "order",
      relatedId: orderID,
      recipient: env.CONTACT_TO,
    });
...
<body style="margin:0;padding:0;background:${PAPER};">
  ...
  try {
    ...
    await logEmailSend(env, {
      resendEmailId: sent && sent.id,
      emailType: "customer_status_update",
      relatedType: "order",
      relatedId: orderID,
