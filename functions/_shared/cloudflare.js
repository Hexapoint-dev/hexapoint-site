...
const D1_FREE_LIMITS = { rowsReadPerDay: 5000000, rowsWrittenPerDay: 100000, totalStorageBytes: 5 * 1024 * 1024 * 1024 };
...
// Actual on-disk size of the D1 database, via the "get database" REST
// endpoint -- separate call from getD1Usage() above because it's a
// different Cloudflare API (REST, not GraphQL Analytics) and needs a
// D1:Read token permission the Analytics-only token may not have.
async function getD1StorageSize(env) {
  if (!env.CLOUDFLARE_D1_DATABASE_ID) return null;
  ...
  if (!res.ok || !data || !data.success) {
    throw new Error(`cf_d1_database_info_failed: ${res.status} ${JSON.stringify(data)}`);
  }
  const fileSize = data.result && data.result.file_size;
  if (typeof fileSize !== "number") {
    // Cloudflare has changed this response shape before -- if this throws,
    // the raw result is in the message, which names the field to fix.
    throw new Error(`cf_d1_database_info_unexpected_shape: ${JSON.stringify(data.result)}`);
  }
  return { bytes: fileSize, limit: D1_FREE_LIMITS.totalStorageBytes };
}
